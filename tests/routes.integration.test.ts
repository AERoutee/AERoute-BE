...
describe('OpenAPI runtime contracts', () => {
  it('documents persisted route identifiers and secure trip impact input only', () => {
    ...
    expect(schemas.RouteOption.properties).toHaveProperty('navigationSteps')
    expect(schemas.NavigationStep.required).toEqual(['instruction', 'travelMode'])
    ...
    expect(schemas.RouteOption.properties).toMatchObject({ dataQuality: { enum: ['modeled_estimate', 'partial_estimate', 'unavailable'] }, averagePm25: { type: ['number', 'null'] }, estimatedExposureIndex: { type: ['number', 'null'] }, composition: { const: 'PROVIDER_SEGMENTS' }, scheduleStatus: { const: 'SCHEDULE_VALIDATED' }, limitations: { type: 'array' } })
    expect(schemas.RouteComparison.properties).toMatchObject({ cleanestDeparture: { type: ['integer', 'null'] } })
